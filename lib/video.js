...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	var execCommand = function (callback, folder) {
		...
		var finalCommands = ['ffmpeg -threads 1 -i']
	 ...
	 */
	this.fnExtractFrameToJPG = this.fnExtractFrameToIMG = function (/* destinationFolder, settings, callback */) {
		...
		settings.file_name = path.basename(settings.file_name, path.extname(settings.file_name)) + '_%d.' + settings.ext;
