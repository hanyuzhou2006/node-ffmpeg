...
/**
 * Calculate the duration in milliseconds from the string retrieved by the ffmpeg info
 */
module.exports.durationToMilliseconds = function (duration) {
	const parts = duration.split(':');
	const secondparts = parts[2].split('.');
	return parseInt(parts[0], 10) * 3600 * 1000 + parseInt(parts[1], 10) * 60 * 1000 + parseInt(secondparts[0], 10) * 1000 + parseInt(pad(secondparts[1], 3), 10);
}


function pad(source, n) {
