	...
	var _videoInfo = function (fileInput, settings) {
		...
		utils.exec([ffmpeg.bin,'-i', utils.addQuotes(fileInput),'2>&1'], settings, function (error, stdout, stderr) {
			...
			var filename		= /from \'(.*)\'/.exec(stdout) || []
			  ...
			  , bitrate         = /bitrate: ([0-9]+) kb\/s/.exec(stdout) || []
			  
			  , container		= /Input #0, ([a-zA-Z0-9]+),/.exec(stdout) || []
			  , video_bitrate	= /Video:.* ([0-9]+) kb\/s/.exec(stdout) || []
			  , video_stream	= /Stream #([0-9\.]+)([a-z0-9\(\)\[\]]*)[:] Video/.exec(stdout) || []
			  , video_codec		= /Video: ([\w]+)/.exec(stdout) || []
			  , resolution		= /(([1-9][0-9]{0,4})x([1-9][0-9]{0,4}))/.exec(stdout) || []
			...
			let pix_fmt = [];
			const findStart = video_codec.length ? video_codec.index + video_codec[1].length : -1;
			let parentheses = 0;
			let start = -1;
			if(findStart != -1){
				for(let i = findStart; i < stdout.length; i++){
					if(stdout[i] === ',' && parentheses === 0){
						start = i;
						break;
					}
					if(stdout[i] === '('){
			...
			if(start > 0){
				pix_fmt = /([\w]+)/.exec(stdout.substring(start)) || []
			}
			// Build return object
			var ret = { 
				filename		: filename[1] || ''
			  ...
			  , duration		: {
                  ...
                  , milliseconds: duration[1] ? utils.durationToMilliseconds(duration[1]) : 0
				}
			  , bitrate: (bitrate.length > 1) ? parseInt(bitrate[1], 10) : 0
			  , video			: {
